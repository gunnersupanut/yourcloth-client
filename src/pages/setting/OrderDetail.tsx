...
// --- 1. Mock Data (เอาไว้เทสก่อนต่อ Service) ---
const mockOrder = {
  orderId: 1014,
  status: "PENDING", // ลองเปลี่ยนเล่นดู: PENDING, INSPECTING, PACKING, SHIPPING, COMPLETE
  orderedAt: "2026-01-19T04:20:44.650Z",
  totalAmount: 2270,
  shippingCost: 50,
  receiver: {
    name: "Gunner (Noodleboy)",
    phone: "089-999-9999",
    address: "88/88 The Base Condo, Bang Sue, Bangkok 10800",
  },
  items: [
    {
      name: "Black T-Shirt (New Collection)",
      quantity: 2,
      price: 500,
      image:
...
export default function OrderDetail() {
  const order = mockOrder; // ใช้ Mock ไปก่อน
    ...
    <div className="max-w-5xl mx-auto p-4 space-y-4 font-kanit pb-20">
      ...
      <div className="bg-white p-6 border-b-2 border-primary shadow-sm">
        <p className="font-bold text-primary mb-4 flex items-center gap-2">
          <MapPin className="w-5 h-5 text-yellow-500  text-bodyxl" /> Shipping
          Address
        </p>
        <div className="pl-7">
          <p className="font-bold text-primary">{order.receiver.name}</p>
          <p className="text-primary text-sm mt-1">{order.receiver.phone}</p>
          <p className="text-black mt-2">{order.receiver.address}</p>
        </div>
      </div>
      ...
      <div className="bg-white p-6 border-b-2 border-primary  shadow-sm space-y-6">
        <h3 className="font-bold text-primary mb-2 flex items-center gap-2">
          <Package className="w-5 h-5 text-bodyxl text-yellow-500" /> Product
          Details
        </h3>
          ...
          >
            ...
            <div className="flex flex-1 flex-col md:flex-row md:justify-between md:items-start">
              ...
              <div className="mb-1 md:mb-0 md:pr-4">
                <h4 className="font-bold text-gray-800 line-clamp-2 text-sm md:text-base">
                  {item.name}
                </h4>
              </div>
